...
vcs -full64 -licqueue '-timescale=1ns/10ps' '+vcs+flush+all' '+warn=all' '-sverilog' +incdir+$UVM_HOME/src $UVM_HOME/src/uvm.sv $UVM_HOME/src/dpi/uvm_dpi.cc -CFLAGS -DVCS RTL/design.sv testbench.sv  && ./simv +vcs+lic+wait '+UVM_TESTNAME=spi_interrupt_test'  ; echo 'Creating result.zip...' && zip -r /tmp/tmp_zip_file_123play.zip . && mv /tmp/tmp_zip_file_123play.zip result.zip
